...
def main():
  global jogador_x, jogador_y, energetico_timer, nivel_concluido, tempo_restante, cpr, max_cpr, incremento_cpr, press_count, em_acao
  ...
  def framesFogo(tela, largura, altura, tamanho):
    imagem = pygame.Surface((largura, altura)).convert_alpha()
    imagem.blit(tela, (0,0), (200, 200, largura, altura))
    imagem = pygame.transform.scale(imagem, (largura * tamanho, altura * tamanho))
    # imagem.set_colorkey((0, 0, 0))

    return imagem

  frame_0 = framesFogo(fogo, 30, 30, 3)
